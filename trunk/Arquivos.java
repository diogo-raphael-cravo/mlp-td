package mlptd;

/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */



/**
 * Classe para acesso estático de nomes de arquivos.
 * Por convenção, arquivos de textura de textos têm sempre o nome do texto que contém.
 * Assim, o arquivo do caractere 1 tem nome 1.PNG. Eles se localizam em '...\texturas\nome da fonte\'
 * @author diogo
 */
public class Arquivos {
    public static final String LINK_BASE = "/home/diogo/Área de Trabalho/mlp-td/texturas/";
    public static String LINK_BASE_TEXTURA_SLIMEKING = LINK_BASE+"slimeking/slimeking_";
    public static String ARQUIVO_TEXTURA_MURO = LINK_BASE+"muro2.png";
    public static String ARQUIVO_TEXTURA_CAVEIRA = LINK_BASE+"caveira4.png";
    public static String ARQUIVO_TEXTURA_CAVEIRA_INVERTIDA = LINK_BASE+"caveira42.png";
    public static String ARQUIVO_TEXTURA_GRAMA = LINK_BASE+"grama.jpg";
    public static String ARQUIVO_TEXTURA_LADRILHO = LINK_BASE+"ladrilho.jpg";
    public static String ARQUIVO_TEXTURA_TORRE = LINK_BASE+"torre.png";
    public static String ARQUIVO_TEXTURA_CANHAO = LINK_BASE+"canhao.png";
    public static String ARQUIVO_TEXTURA_MADEIRA = LINK_BASE+"madeira.jpg";
    public static String ARQUIVO_TEXTURA_X = LINK_BASE+"x.png";
    public static String ARQUIVO_TEXTURA_OURO = LINK_BASE+"ouro.png";
    public static String ARQUIVO_TEXTURA_CORACAO = LINK_BASE+"coracao.png";
    public static String ARQUIVO_TEXTURA_RELOGIO = LINK_BASE+"relogio.png";
    public static String ARQUIVO_TEXTURA_FLECHA = LINK_BASE+"flecha.png";
    public static String ARQUIVO_TEXTURA_BALA_DE_CANHAO = LINK_BASE+"balaCanhao.png";



}
